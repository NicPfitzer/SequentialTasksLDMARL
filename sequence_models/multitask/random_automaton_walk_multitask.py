...
NAV_HOME = 9
IDLE = 10
FIND_FIRST_SWITCH = 11
FIND_SECOND_SWITCH = 12
FIND_THIRD_SWITCH = 13
...
STATES = {
    ...
    "purple": FIND_PURPLE,
    "explore": EXPLORE,
    "defend_tight": DEFEND_TIGHT,
    "defend_wide": DEFEND_WIDE,
...
EVENTS: Sequence[str] = {
    ...
    6: ("spotted_adversary"),
    7: ("reached_base"),
    8: ("found_first_switch"),
    9: ("found_second_switch"),
    10: ("found_third_switch"),
}
EVENT_DIM = len(EVENTS)

R, G, B, P, SW, FF, FE, FB, S1, S2, S3 = range(11)
...
# --------------------------------------------------------------------------- #
#  Concrete DFA generator: EXPLORE -> DEFEND_TIGHT -> DEFEND_WIDE
# --------------------------------------------------------------------------- #

def explore_flag_defend(start_id: int = 0) -> DFAConfig:
    """Find flags in RGBP order, hit the switch, and navigate to the goal."""
    
    transition: Dict[State, Callable[[Vector], State]] = {
        "explore":   advance_if(has(FF),  "defend_wide",  "explore"),
        "defend_wide":   advance_if(has(FE),  "defend_tight",   "defend_wide"),
        "defend_tight":   advance_if(not_has(FE),  "defend_wide",   "defend_tight"),
    }

    return DFAConfig(
        name="find_flag_defend_home",
        automaton_id=start_id,
        initial="explore",
        finals={},
        transition=transition,
        outfile=Path("sequence_models/data/capture_flag/random_walks/random_walk_find_flag_defend_home.json"),
    )

# --------------------------------------------------------------------------- #
#  Concrete DFA generator: EXPLORE -> NAV_HOME -> IDLE
# --------------------------------------------------------------------------- #
def explore_flag_navigate_home(start_id: int = 0) -> DFAConfig:
    """Find flags in RGBP order, hit the switch, and navigate to the goal."""

    transition: Dict[State, Callable[[Vector], State]] = {
        "explore":   advance_if(has(FF),  "navigate_home",  "explore"),
        "navigate_home":   advance_case(
            (has(FB),  "idle"),
            (not_has(FF), "explore"),
            default="navigate_home"
        ),
        "idle":   (lambda v: "idle"),
    }

    return DFAConfig(
        name="find_flag_navigate_home",
        automaton_id=start_id,
        initial="explore",
        finals={"idle"},
        transition=transition,
        outfile=Path("sequence_models/data/capture_flag/random_walks/random_walk_find_flag_navigate_home.json"),
    )
    
# --------------------------------------------------------------------------- #
#  Concrete DFA generator: FIRST -> SECOND -> THIRD -> GOAL
# --------------------------------------------------------------------------- #
...
def hit_all_switches_then_goal(start_id: int) -> DFAConfig:
    ...
    transition: Dict[State, Callable[[Vector], State]] = {
        "first":     advance_if(has(S1),  "second",  "first"),
        "second":   advance_if(has(S2),  "third",   "second"),
        "third":    advance_if(has(S3),  "goal", "third"),
        "goal":    (lambda v: "goal"),
    }
...
if __name__ == "__main__":
    ...
    AUTOMATA += [explore_flag_defend(start_id=len(AUTOMATA))]
    AUTOMATA += [explore_flag_navigate_home(start_id=len(AUTOMATA))]
