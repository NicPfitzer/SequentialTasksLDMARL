...
class EventSequenceDataset(Dataset):
    ...
    def __init__(self, raw: List[Dict]):
        ...
        for ix, s in enumerate(raw):
            ...
            if e_key not in s or not all(k in s for k in ("y", "h", "label")):
...
class EventRNN(pl.LightningModule):
    ...
    # ------------------------------------------------------------------
    def rollout(self, e: Tensor, y: Tensor, h: Tensor) -> Tensor:
        _, T, _ = e.shape
    ...
    def _initalized_rollout(self, e: Tensor, y: Tensor, h_gt: Tensor) -> Tensor:
        B, T, _ = e.shape
        D = self.hparams.latent_dim
        L = self.hparams.num_layers
        
        h = self.subtask_encoder(h_gt)
    ...
    def _step(self, batch):
        ...
        loss = self.cls_w * loss_cls.clone()

        return loss, loss_cls, loss_recon_h


    # ----------------------------§--------------------------------------
    def training_step(self, batch: Dict[str, Tensor], batch_idx: int):
        loss, loss_cls, loss_recon_h = self._step(batch)
        self.log_dict({
            "train_loss": loss,
...
def run_training(batch_size: int, input_dim: int, resume_ckpt: str | None = None, decoder_path: str = "sequence_models/four_rooms/four_rooms_decoder.pth"):
    train_loader, val_loader = make_loaders(
        "sequence_models/data/dataset_multitask.json", batch_size=batch_size
    )
    ...
    save_path = f"sequence_models/multitask/event_rnn_best_{run_name}.pth"
