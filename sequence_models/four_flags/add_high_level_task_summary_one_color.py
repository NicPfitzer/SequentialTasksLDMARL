...
SENTENCE_OVERRIDE_BY_FILE: Dict[str, str] = {
    ...
    "dataset_no_summary_random_walk_blue_goal":    "Find the blue flag, spot the switch, and head for the target.",
...
FNAME_PATTERN = re.compile(
    r"(?P<c>red|green|blue|purple)_goal",
    re.IGNORECASE,
)

def infer_color_from_filename(p: Path) -> Optional[Tuple[str, str]]:
...
def build_prompt_template(color1: str) -> str:
    return textwrap.dedent(f"""
        Write ONE short imperative sentence (≤16 words) that states:
        first locate the {color1} flag, then the switch, then go to the goal.
...
def stream_process():
    ...
    with OUTPUT_JSON.open(mode, encoding="utf-8") as dst:
        ...
        for f_idx, input_path in enumerate(files):
            ...
            # Determine colors
            color = infer_color_from_filename(input_path)
            if color is None:
                raise ValueError(f"Could not infer color for {input_path.name}")
            c1 = color

            # Build prompt template
            prompt_template = build_prompt_template(c1)
