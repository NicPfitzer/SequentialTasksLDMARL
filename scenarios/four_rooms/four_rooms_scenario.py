...
from scenarios.four_rooms.language import LanguageUnit, load_sequence_model, FIND_FIRST_SWITCH, FIND_SECOND_SWITCH, FIND_THIRD_SWITCH, FIND_GOAL
...
class FourRoomsSwitchesScenario(BaseScenario):
    ...
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.goal_radius = getattr(self, "goal_radius", None) or self.agent_radius
    ...
    def _set_initial_gate_states(self, env_index: int = None):
        # Open gates <= p. Closed gates remain collidable at their divider x.
        # p: 0->none open, 1->gate1 open, 2->gates1-2 open, 3->all open
        
        def p_of(idx):
            p_val = int(self.language_unit.states[idx].item())
            return max(0, min(3, p_val))

        if env_index is None:
            self._gates_open.zero_()
            for b in range(self.world.batch_dim):
                p = p_of(b)
                self._gates_open[b, :p] = True
        else:
            p = p_of(env_index)
            self._gates_open[env_index].zero_()
            if p > 0:
                self._gates_open[env_index, :p] = True

    def _spawn_agents_for_env(self, b: int):
        # Spawn in accessible space up to the next closed gate
        p = int(self.language_unit.states[b].item())
        p = max(0, min(3, p))
        # Accessible rooms: [0 .. min(p, 2)] because we never spawn directly in room 3
        rightmost_room = min(p, 2)
        xmin = self.room_bounds[0][0]
        ...
        for a in self.world.agents:
            ...
            self.set_goal_coords(a, self.world, env_index=b)
    ...
    def done(self):
        # Success when the current subtask is completed.
        # If require_all_to_finish=False, any agent is enough; else require all agents.
        p = self.language_unit.states.long().clamp_min(0).clamp_max(3)  # [B]
        if self.require_all_to_finish:
            # Build per-env "all agents finished" signals
            # For switches, require all agents to overlap the active switch at least once
            per_agent_switch = torch.stack([a.switch_hits for a in self.world.agents], dim=1)  # [B, N, 3]
            all_switch = per_agent_switch.all(dim=1)  # [B,3]
            per_agent_goal = torch.stack([a.on_goal for a in self.world.agents], dim=1).all(dim=1)  # [B]
            success_table = torch.stack([all_switch[:, 0], all_switch[:, 1], all_switch[:, 2], per_agent_goal], dim=1)  # [B,4]
        else:
            # Any agent suffices -> use team switches and any-goal
            any_goal = torch.stack([a.on_goal for a in self.world.agents], dim=1).any(dim=1)  # [B]
            success_table = torch.stack([self.team_switch_hits[:, 0],
                                         self.team_switch_hits[:, 1],
                                         self.team_switch_hits[:, 2],
                                         any_goal], dim=1)  # [B,4]
        b = torch.arange(self.world.batch_dim, device=self.world.device)
        idx = p  # p maps 0->S1, 1->S2, 2->S3, 3->GOAL
        return success_table[b, idx]
    ...
    def extra_render(self, env_index: int = 0):
            ...
            geoms = []
            task_state = self.language_unit.states[env_index].item()
